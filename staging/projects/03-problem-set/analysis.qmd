...
Draw some histograms (`geom_histogram()`) and scatterplots (`geom_point()`) to investigate the data. Which columns in the table does it make sense to represent by color, or to facet by? Choose one of these variables and, in one or more chunks below, draw some plots with them. Say what it tells you about the `penguins` data.
