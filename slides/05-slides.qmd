...
```{r}
nyc_license |> 
  summarize(total = n(), .by = extract_year)
```
...
```{r}
nyc_license |> 
  summarize(.by = c(borough, extract_year))
```
...
```{r}
nyc_license |> 
  summarize(total = n(), .by = c(borough, extract_year))
```
...
nyc_license |> 
  summarize(total = n(), .by = c(borough, extract_year)) |> 
  pivot_wider(names_from = extract_year, values_from = total) |> 
  knitr::kable()
```
...
nyc_license |> 
  ...
  summarize(total = n(), .by = c(animal_name)) |> 
