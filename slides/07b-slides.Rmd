...
`r chunk_reveal("reveal-nccensus", widths = c(40,60), title = "# Get and widen the data")`

```{r reveal-nccensus, include = FALSE}
nc %>% 
...
```{r reveal-nccensus-obj, include = FALSE}
nc_wide <- nc %>% 
  left_join(popvars, by = "variable") %>% 
  relocate(name, .after = variable) %>% 
  select(-variable) %>% 
...
.pull-left.w45[
```{r codefig-hispanicplot, message=FALSE, fig.show="hide", fig.width=4.8, fig.height=4.5}
nc_wide %>%
