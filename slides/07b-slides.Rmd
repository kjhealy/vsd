...
# .kjh-yellow[Working with<br />] .kjh-lblue[`tidycensus`] 
...
# Some Data by County for North Carolina
...
```{r reveal-nccensus-obj, eval = TRUE, include = FALSE}
nc_wide <- nc %>% 
  left_join(popvars, by = "variable") %>% 
  relocate(name, .after = variable) %>% 
  select(-variable) %>% 
...
.pull-left.w45[
```{r codefig-hispanicplot, message=FALSE, fig.show="hide", fig.width=4.8, fig.height=5}
nc_wide %>%
...
.pull-left.w45[
```{r codefig-hispvpop, message=FALSE, fig.show="hide", fig.width=4.8, fig.height=4.5}
nc_wide %>%
...
.pull-left.w45[
```{r codefig-blackvpop, message=FALSE, fig.show="hide", fig.width=4.8, fig.height=4.5}
nc_wide %>%
  ggplot(mapping = aes(x = summary_value,
                       y = nh_black_pct)) + 
  geom_point() + 
...
.pull-left.w45[
```{r codefig-blackvwhite, message=FALSE, fig.show="hide", fig.width=4.8, fig.height=4.5}
nc_wide %>%
...
.pull-left.w45[
```{r codefig-usapyrbasic, message=FALSE, fig.show="hide", fig.width=6, fig.height=5.5}
usa_pyr |> 
...
.pull-left.w45[
```{r codefig-usapyr, message=FALSE, fig.show="hide", fig.width=6, fig.height=5.5}
usa_pyr |> 
  ggplot(mapping = aes(x = value, 
                       y = AGEGROUP, 
                       fill = SEX)) + 
  geom_col(width = 0.95, alpha = 0.75) +
  # In the two scale functions we use ~ label_number() 
  # and ~ str_remove_all() as anonymous functions. Slightly 
  # more advanced usage than normal ...
  scale_x_continuous(
    labels = ~ label_number(scale = 0.001, suffix = "k")(abs(.x))) + 
  scale_y_discrete(
    labels = ~ str_remove_all(.x, "Age | years")) + 
  labs(x = "Count", 
       y = NULL, 
       title= "US Population Pyramid") + 
...
# .middle.squish4.huge.kjh-orange[Midterm Postmortem]
