...
# .kjh-green[Iteration], .kjh-yellow[Missingness], and .kjh-lblue[Selection Effects]
...
.SMALL[Say we have a local folder of data named `congress/`]
...
# .kjh-green[`read_csv()`] can do this directly 
...
# Using .kjh-yellow[naniar] and .kjh-yellow[visdat]
...
.top[![:scale 80%](img/10-survivorship-plane.png)]
...
.top[![:scale 100%](img/10-feldon-01.png)]
...
.top[![:scale 100%](img/10-feldon-02.png)]
...
.top[![:scale 100%](img/10-feldon-03.png)]
...
# .middle.squish4.huge[.kjh-red[Problems with this approach]]
...
df <- df |>
  mutate(across(all_of(num_vars), ~ as.numeric(.x)))
```
