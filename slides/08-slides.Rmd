...
# A Time Series: .kjh-yellow[US Monthly Births]
...
# Moving Averages with .kjh-yellow[`slider`]
...
# A Moving Average, order 7

```{r ma-boom-7, fig.show='hide', fig.width=16, fig.height=4}
boom |> 
  mutate(
    mav = slide_dbl(births, mean,
                    .before = 3, .after = 3, 
...
- .small[As the order goes up, the window for the average widens, and the line gets smoother and smoother.]
...
- .small[When the window is the same as the seasonal period, then all the seasonality gets averaged away.] 
...
# A Moving Average, order 60 (i.e. 5 years)

.panelset[

.panel[.panel-name[Code]
```{r ma-boom-60, fig.show='hide', fig.width=16, fig.height=4}
boom |> 
  mutate(
    mav12 = slide_dbl(births, mean,
                    .before = 29, .after = 30, 
...
- .small[The wider the window, the flatter the line. We also lose observations at each end.]
...
# .kjh-green[Seasonality] in US Birth Rates
...
#### A Trend Part: a moving average, $\hat{T}$ 
...
# Calculate the .kjh-yellow[Trend] part
...
# Calculate the .kjh-yellow[Seasonal] part
...
# Calcualte the .kjh-yellow[Seasonal] part
...
# Claculate the .kjh-yellow[Seasonal] part

- In a "Classical" decomposition the Seasonal part just repeats. 

```{r seasonal-rep, fig.show='hide', fig.width=16, fig.height=1.2}
boom_ts |> 
...
# Calculate the .kjh-yellow[Remainder] part
...
# Decomposition: .kjh-yellow[`y = t + s + r`] 
...
# The .kjh-green[STL] Decomposition
...
### Default monthly trand window is 21
...
```{r autoplot-stl-panel, fig.show='hide', fig.width=16, fig.height=6}
boom |> 
  mutate(date = yearmonth(date)) |> 
  as_tsibble(index = "date") |> 
  model(
...
```{r autoplot-stl-panel-2, fig.show='hide', fig.width=15, fig.height=9}
bc |> 
...
```{r}
library(lubridate)
s30s <- interval(ymd(19330101), ymd(19380101))
s50s <- interval(ymd(19530101), ymd(19580101))
s00s <- interval(ymd(20030101), ymd(20080101))
...
p_out <- bc_int |> 
  ...
  labs(x = "Month",  y = "Seasonal Component of the Birth Rate", 
       title = "Changing Seasonality: Three Five-Year periods in Three Decades")
```
...
```{r, fig.width=15, fig.height=8.25, echo = FALSE}
p_out
```
